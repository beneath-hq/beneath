...
public class DryWriter extends AIODelayBuffer<InstanceRecordAndSize> {
  ...
  protected void flush() {
    ...
    System.out.println(String.format("Flushing %i buffered records", this.records.size()));
    for (InstanceRecordAndSize r : records) {
      System.out.println(String.format("Flushed record (table=%s, size=%i bytes): %s",
          r.instance.table.identifier.toString(), r.size, r.record));
      this.total += 1;
    }
    System.out.println(String.format("Flushed %i records (%i total during session)", this.records.size(), this.total));
  }
