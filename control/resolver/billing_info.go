...
func (r *mutationResolver) UpdateBillingInfoBillingPlan(ctx context.Context, organizationID uuid.UUID, billingPlanID uuid.UUID) (*entity.BillingInfo, error) {
	organization := entity.FindOrganization(ctx, organizationID)
	if organization == nil {
		return nil, gqlerror.Errorf("Organization %s not found", organizationID)
	}
	...
	newBillingInfo, err := billingInfo.UpdateBillingPlan(ctx, billingPlanID)
	if err != nil {
