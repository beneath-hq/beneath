...
func (s *gRPCServer) QueryLog(ctx context.Context, req *pb.QueryLogRequest) (*pb.QueryLogResponse, error) {
	...
	if len(cursor) == 0 {
		...
		cursor, err = db.Engine.QueryLog(ctx, stream, stream, stream, where)
		if err != nil {
	...
	// get result iterator
	it, err := db.Engine.ReadLog(ctx, stream, stream, stream, cursor, int(req.Limit))
	if err != nil {
		return nil, grpc.Errorf(codes.InvalidArgument, "%s", err.Error())
	}

	// make response
	response := &pb.QueryLogResponse{}
	bytesRead := 0
