...
func (s *gRPCServer) QueryWarehouse(ctx context.Context, req *pb.QueryWarehouseRequest) (*pb.QueryWarehouseResponse, error) {
	res, err := api.HandleQueryWarehouse(ctx, &api.QueryWarehouseRequest{
		Query:           req.Query,
...
func (s *gRPCServer) PollWarehouseJob(ctx context.Context, req *pb.PollWarehouseJobRequest) (*pb.PollWarehouseJobResponse, error) {
	...
	res, err := api.HandlePollWarehouse(ctx, &api.PollWarehouseRequest{
		JobID: jobID,
	})
	if err != nil {
		return nil, err.GRPC()
...
func makeWarehouseJobPB(job *api.WarehouseJob) *pb.WarehouseJob {
