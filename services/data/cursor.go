...
// utility to wrap an engine cursor with Cursor
func wrapCursor(cursorType CursorType, id uuid.UUID, engineCursor []byte) []byte {
	if len(engineCursor) == 0 {
		return engineCursor
	}
	return NewCursor(cursorType, id, engineCursor).GetBytes()
}
