  ...
  return (
    <Container maxWidth="md" className={classes.container}>
      <Alert severity="info">
        ...
        {!me && (
          <>
            You'll first have to <Link href="/-/auth">login or signup</Link> to get a secret for connecting to Beneath
            (don't worry, it's free and we won't share your data with anyone)
          </>
        )}
      </Alert>
      <VSpace units={2} />
          ...
          <TabContext value={language}>
            ...
            >
              {languageTabs.map((tab) => (
                <Tab key={tab} label={tab} value={tab} className={classes.tab} />
              ))}
            </Tabs>
          </TabContext>
        </Grid>
      ...
      <Grid item>
        {language === "Python" && pythonDetail === "Setup" && (
          <>
            ...
            <CodeBlock language={"bash"}>{`pip install beneath`}</CodeBlock>
            <VSpace units={2} />
            <Typography variant="body1" paragraph>
              Authenticate your environment:
            </Typography>
            <CodeBlock language={"bash"}>{`beneath auth SECRET`}</CodeBlock>
          </>
        )}
...
df = await beneath.easy_read("${toURLName(stream.project.organization.name)}/${toURLName(
                stream.project.name
...
stream = await client.create_stream(stream_path="${toURLName(stream.project.organization.name)}/${toURLName(
                stream.project.name
              )}/${toURLName(stream.name)}", schema=schema)
...
beneath.easy_consume_stream(
  input_stream_path="${toURLName(stream.project.organization.name)}/${toURLName(stream.project.name)}/${toURLName(
                stream.name
              )}",
...
beneath.easy_derive_stream(
  ...
  output_stream_schema=OUTPUT_SCHEMA
            ...
            <CodeBlock language={"bash"}>{`python your_pipeline.py stage USERNAME/PROJECT/SERVICE`}</CodeBlock>
            <VSpace units={2} />
            <Typography variant="body1" paragraph>
              Lastly, run your pipeline:
            </Typography>
            <CodeBlock language={"bash"}>{`python your_pipeline.py run USERNAME/PROJECT/SERVICE`}</CodeBlock>
          </>
        )}
        {language === "Javascript" && javascriptDetail === "Setup" && (
          <>
            ...
            <CodeBlock language={"bash"}>{`npm install beneath`}</CodeBlock>
            <VSpace units={2} />
            <Typography variant="body1" paragraph>
              Or install with yarn:
            </Typography>
            <CodeBlock language={"bash"}>{`yarn install beneath`}</CodeBlock>
          </>
        )}
        {language === "Javascript" && javascriptDetail === "Reading" && (
          <>
            <Typography variant="body1" paragraph>
              You can query this stream directly from your frontend. It's very important to use permissioned "read-only"
              secrets in your frontend code.
            </Typography>
            <CodeBlock language={"javascript"}>
              {`fetch("${GATEWAY_URL}/v1/${toURLName(stream.project.organization.name)}/${toURLName(
                stream.project.name
              )}/${toURLName(stream.name)}", {
    ...
    console.log(data)
        ...
        {language === "React" && reactDetail === "Setup" && (
          <>
            ...
            <CodeBlock language={"bash"}>{`npm install beneath-react`}</CodeBlock>
            <VSpace units={2} />
            <Typography variant="body1" paragraph>
              Or install with yarn:
            </Typography>
            <CodeBlock language={"bash"}>{`yarn install beneath-react`}</CodeBlock>
          </>
        )}
...
const App = () => {
  const { records, error, loading } = useRecords({
    stream: "${toURLName(stream.project.organization.name)}/${toURLName(stream.project.name)}/${toURLName(
                stream.name
              )}",
    ...
    pageSize: 500,
        ...
        {language === "SQL" && (
          <>
            ...
            <CodeBlock language={"sql"}>{`select * from \`${toURLName(stream.project.organization.name)}/${toURLName(
              stream.project.name
        ...
        {language === "cURL" && (
          <>
            ...
            <CodeBlock language={"bash"}>
              {`curl -H "Authorization: Bearer SECRET" ${GATEWAY_URL}/v1/${toURLName(
