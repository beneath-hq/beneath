...
const ViewSecrets: FC<ViewSecretsProps> = ({ userID }) => {
  return (
    <List dense={true}>
      <Query<SecretsForUser, SecretsForUserVariables> query={QUERY_USER_SECRETS} variables={{ userID }}>
        {({ loading, error, data }) => {
          if (loading) {
          ...
          return data.secretsForUser.map(({ createdOn, description, secretID, prefix }) => (
